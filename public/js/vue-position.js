    ...
    axios.get('/data/position.json')
        .then((resp) => {
            ...
            simpleQuote.subscribe('nsp-vue-position', subCodes, (newQuote) => {
                console.log('(非全部ready)有新的行情推送至客户端:', newQuote);
                ...
                var app = new Vue({
                    ...
                    methods: {
                        price: function(row) {
                            return gt(row.code + '.' + row.market);
                        },
                        positionPrice: function(row) {
                            return this.price(row) * row.position;
                        }
                    }
                });

                simpleQuote.nsps['nsp-vue-position'].onNewQuote = newQuote => {
                    console.log('(已全部ready) 有新的行情推送至客户端:', newQuote);
                };
            });
